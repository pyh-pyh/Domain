...
def generate_defect_data(N, defectype=['I', 'V']):
    ...
    with open('data/generate.txt', 'w') as f:
        ...
        for i in range(N):
            ...
            print('%d %s %f %f %f' %
                  (i + 1, defectype[flag], x * bound[1], y * bound[3], z * bound[5]),
                  file=f)
...
def read_reaction(filename):
    ...
    with open('config/' + filename + '/Reaction.txt', 'r') as f:
        ...
        while i < len(reaction):
            if reaction[i] != '\n':
                Ere[reaction[i].split()[0]] = [
                    reaction[i].split()[1], float(reaction[i].split()[2])
                ]
                i += 1
            else:
                break
    ...
    with open('config/Box.txt', 'r') as f:
        ...
        while i < len(box):
            if box[i].strip() == 'box size':
                gl.set_value('bound', [
                    float(box[i + 1].split()[1]),
                    float(box[i + 1].split()[2]),
                    float(box[i + 2].split()[1]),
                    float(box[i + 2].split()[2]),
                    float(box[i + 3].split()[1]),
                    float(box[i + 3].split()[2])
                ])
                i += 3
                continue
                ...
                while i + 1 < len(box):
                    if box[i + 1] != '\n':
                        if box[i + 1].strip() != 'z = ' + str(z):
                            strip = [
                                int(box[i + 1].split()[j]) for j in range(len(box[i + 1].split()))
                            ]
                            panel.append(strip)
                            i += 1
                        else:
...
def run_simulation(simulation_time):
    ...
    while t <= simulation_time:
        ...
        # Collect rates
        if init == 0:
            Rm = [[defect[i].rate(temperature)[0], i, i] for i in range(len(defect))
                  if defect[i].rate(temperature)[0] != 0]
            # print('Rm', len(Rm))
            Rbk = [[defect[i].rate(temperature)[1], i] for i in range(len(defect))
                   if defect[i].rate(temperature)[1] != 0]
            # print('Rbk', len(Rbk))
            Rft = [[defect[i].rate(temperature)[2], i] for i in range(len(defect))
                   if defect[i].rate(temperature)[2] != 0]
            # print('Rft', len(Rft))
            RN = [Rm[i][0] for i in range(len(Rm))] + [Rbk[i][0] for i in range(len(Rbk))] + [
                Rft[i][0] for i in range(len(Rft))
            ] + [Rre[i][0] for i in range(len(Rre))] + [Rint[i][0] for i in range(len(Rint))]
            gl.set_value('Rm', Rm)
        ...
        while True:
            ...
            if (sum(RN[:point]) <= r * sum(RN)) and (sum(RN[:(point + 1) %
                ...
                if point < len(Rm):
                    j = Rm[point][2] + 1
                    idj = updateID(j)
                    print('defect %d %s migrate' % (defect[idj].ID, defect[idj].defect_type),
                          (defect[idj].x, defect[idj].y, defect[idj].z), t)
                    mig += 1
                    migrate(point)

                elif (point >= len(Rm)) and (point < len(Rm) + len(Rbk)):
                    j = Rbk[point - len(Rm)][1]
                    print('defect %d %s break up' % (defect[j].ID, defect[j].defect_type),
                          (defect[j].x, defect[j].y, defect[j].z), t)
                    brk += 1
                    break_up(point)

                elif (point >= len(Rm) + len(Rbk)) and (point < len(Rm) + len(Rbk) + len(Rft)):
                    j = Rft[point - len(Rbk) - len(Rm)][1]
                    print('defect %d %s F-T' % (defect[j].ID, defect[j].defect_type),
                          (defect[j].x, defect[j].y, defect[j].z), t)
                    FT += 1
                    franck_turnbull(point)

                elif point >= len(Rm) + len(Rbk) + len(Rft) and (
                        point < len(Rm) + len(Rbk) + len(Rft) + len(Rre)):
                    idj = Rre[point - len(Rbk) - len(Rm) - len(Rft)][1] + 1
                    idk = Rre[point - len(Rbk) - len(Rm) - len(Rft)][2] + 1
                    j = updateID(idj)
                    k = updateID(idk)
                    print(
                        'defect %d %s, %d %s recombine' %
                        (defect[j].ID, defect[j].defect_type, defect[k].ID, defect[k].defect_type),
                        (defect[j].x, defect[j].y, defect[j].z),
                        (defect[k].x, defect[k].y, defect[k].z), t)
                    re += 1
                    recombine(point)
                    ...
                    print(
                        'defect %d %s absorbed by interface %d %s' %
                        (defect[j].ID, defect[j].defect_type, interface[k].ID,
                         interface[k].defect_type), (defect[j].x, defect[j].y, defect[j].z),
                        (interface[k].x, interface[k].y, interface[k].z), t)
                    ab += 1
                    absorb(point)
